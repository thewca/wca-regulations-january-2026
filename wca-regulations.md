...
- B2) Starting the attempt:
    ...
    - B2d) When using the speedsolving timer, the competitor should keep their hands on the timer until they see the timer ready indicator. Indicator details are included in the [Approved Timers List](link). The timer is started when the competitor removes their hand(s) from the timer. The competitor starts the attempt and the solve by starting the timer.
